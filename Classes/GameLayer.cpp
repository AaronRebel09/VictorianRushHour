...
void GameLayer::resetGame () {
    ...
    _scoreDisplay->setString ( StringUtils::format("%i", (int)_score) );
            ...
            case kGamePlay:
                ...
                if (_player->getState() == kPlayerFalling) {
                    ...
                    if (_player->getState() !=  kPlayerDying) {
                        int idJump = AudioEngine::play2d( "jump.wav" );
                        AudioEngine::setVolume(    idJump, 0.99f);
...
void GameLayer::createGameScreen () {
    ...
    for(int i = 1; i <= 3; i++) {
        auto frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("jam_%i.png", i));
